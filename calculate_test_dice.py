...
for case in cases:
    print(f"\n--- Case: {case} ---")
    try:
        pred_native = sitk.ReadImage(f'temp_cartimorph_io/output_test/{case}.nii.gz')
        gt_iso = sitk.ReadImage(f'temp_cartimorph_io/gt_test/{case}_label.nii.gz')
        ...
        for label_val, label_name in labels_dict.items():
            ...
            print(f"{label_name:25s} | Dice: {dice:.4f}")
...
for label_name, scores in all_dices.items():
    ...
    print(f"{label_name:25s} | Mean Dice: {avg:.4f}")
