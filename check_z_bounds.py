...
def get_z_bounds(mask):
    z_indices = np.where(mask.any(axis=(1,2)))[0]
    if len(z_indices) == 0: return None
    return z_indices[0], z_indices[-1]

our_z = get_z_bounds(our_femur)
ts_z = get_z_bounds(ts_femur)

print(f"Our femur Z-bounds: {our_z}")
print(f"TS femur Z-bounds: {ts_z}")
